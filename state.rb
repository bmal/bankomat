...
class State
    def initialize(info)
        @console = info[:console] if info.keys.include? :console
        @accounts = info[:accounts] if info.keys.include? :accounts
    end
...
class UserCreationState < State
    def receive_request(_logged_user)
        ...
        account_number = @accounts.instance.add_account_and_return_its_number(user)
...
class AttemptToLoggedInState < State
    def receive_request(_logged_user)
        ...
        _, found_user = @accounts.instance.find { |account_number, _| account_number == parsed_account_number }
...
class LoggedInState < State
    def receive_request(logged_user)
...
class TransferState < State
    def receive_requres(logged_user)
...
class StateFactory
    ...
    def initialize(console, accounts)
        @logged_out_state = LoggedOutState.new(console: console, accounts: accounts)
        @logged_in_state = LoggedInState.new(console: console)
        @attempt_to_logged_in_state = AttemptToLoggedInState.new(console: console, accounts: accounts)
        @user_creation_state = UserCreationState.new(console: console, accounts: accounts)
        @ending_state = EndingState.new(console: console)
        @deposite_in_state = DepositeInState.new(console: console)
        @withdraw_out_state = WithdrawOutState.new(console: console)
        @transfer_state = TransferState.new(console: console, accounts: accounts)
    end
end
